    ...
    for model_name in AVAILABLE_MODELS:
        try:
            ...
            if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
                logger.warning(f"FAILURE with model {model_name}: 429 Rate Limit / Quota Exceeded. Switching to next available model...")
            else:
                # Truncate other generic errors to 150 characters to prevent log spam
                short_error = error_msg if len(error_msg) <= 150 else error_msg[:147] + "..."
                logger.warning(f"FAILURE with model {model_name}: {short_error}. Switching to next available model...")
